...
const ImpactPage: React.FC = () => {
  const memories = [
    { title: "Waste Segregation - Day 5", icon: <FaRecycle size={60} className="text-white" />, bg: "from-green-400 via-emerald-500 to-green-700" },
    { title: "Community Clean-up", icon: <FaLeaf size={60} className="text-white" />, bg: "from-emerald-400 via-lime-500 to-green-600" },
    { title: "First Sapling Planted", icon: <FaTree size={60} className="text-white" />, bg: "from-sky-400 via-blue-500 to-cyan-600" },
  ];
  ...
  return (
    <div className="relative min-h-screen overflow-hidden font-sans">
      ...
      <motion.div
        ...
        style={{ background: "radial-gradient(circle at 20% 30%, #bbf7d0, transparent 60%), radial-gradient(circle at 80% 70%, #bfdbfe, transparent 60%), radial-gradient(circle at 50% 50%, #fde68a, transparent 70%)" }}
      ...
      <div className="relative container mx-auto p-6 pt-28 z-10">
        ...
        >
          ...
          <div className="lg:col-span-3 grid grid-cols-3 gap-6 items-center">
            {[{ value: 52, label: "5.2kg Plastic Saved", color: "emerald" }, { value: 78, label: "12.8kg CO₂ Reduced", color: "blue" }, { value: 35, label: "350L Water Conserved", color: "orange" }].map((stat, i) => (
              <motion.div key={i} whileHover={{ scale: 1.08 }} className="text-center">
                <CircularProgress progress={stat.value} size={120} strokeWidth={10} color={stat.color} />
        ...
        <motion.div className="mt-16" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
          ...
          <div className="bg-white/95 backdrop-blur-md p-10 rounded-3xl shadow-xl border border-gray-200 flex justify-center">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-y-8 gap-x-6">
              {["yellow", "sky", "green", "pink"].map((color, i) => (
                <motion.div
                  key={i}
                  whileHover={{ rotate: [0, 10, -10, 0], scale: 1.15 }}
                  className={`w-20 h-20 bg-${color}-400 rounded-full shadow-lg mx-auto flex items-center justify-center`}
                >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
            {memories.map((mem, i) => (
              ...
              >
                ...
                <p className="font-bold text-lg text-center drop-shadow-md">{mem.title}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
