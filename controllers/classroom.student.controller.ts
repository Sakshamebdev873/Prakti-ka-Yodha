...
export const joinClassroom = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const classroomUser = await prisma.classroomUser.upsert({
      where: {
...
export const getMyClassrooms = async (req: Request, res: Response) => {
  const studentId = req.user?.profileId;
  try {
    const classroomEnrollments = await prisma.classroomUser.findMany({
      where: { studentId },
    ...
    const classrooms = classroomEnrollments.map(
...
export const getClassroomDetails = async (req: Request, res: Response) => {
  const studentId = req.user?.profileId as string;
  const { classroomId } = req.params;
  try {
    const enrollment = await prisma.classroomUser.findUnique({
