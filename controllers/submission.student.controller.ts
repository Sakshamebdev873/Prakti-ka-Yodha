 ...
 */
export const startProjectForChallenge = async (req: Request, res: Response) => {
    ...
    try {
        const project = await prisma.$transaction(async (tx) => {
            ...
            await tx.userChallenge.upsert({
                ...
                create: { studentId, challengeId, status: ChallengeStatus.PENDING },
                update: {},
            });
...
export const createSubmission = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const existingPendingSubmission = await prisma.submission.findFirst({
            where: {
                projectId,
                studentId,
        ...
        const approvedSubmission = await prisma.submission.findFirst({
            where: { projectId, studentId, status: SubmissionStatus.APPROVED }
        });
