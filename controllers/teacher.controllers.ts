...
export const createClassroom = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error creating classroom.", error });
  }
};
...
export const getMyClassrooms = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error fetching classrooms.", error });
  }
};
...
export const getClassroomDetails = async (req: Request, res: Response) => {
  ...
  try {
    const classroom = await prisma.classroom.findFirst({
      ...
      include: {
        ...
        students: {
          select: {
            student: {
              // Navigate through the ClassroomUser to the Student profile
              select: {
                id: true,
                name: true,
                ecoScore: true,
    ...
    return res
      .status(500)
      .json({ message: "Server error fetching classroom details.", error });
  }
};
/**
 * @route   PUT /api/teacher/classrooms/:classroomId
 * @desc    Updates the name of a classroom owned by the teacher.
...
export const updateClassroom = async (req: Request, res: Response) => {
  ...
  if (!name || typeof name !== "string" || name.length < 3) {
    return res
      .status(400)
      .json({ message: "Classroom name must be at least 3 characters long." });
  }
  try {
    ...
    if (error.code === "P2025") {
      return res.status(404).json({
        message:
          "Classroom not found or you do not have permission to edit it.",
      });
    }
    res
      .status(500)
      .json({ message: "Server error updating classroom.", error });
  }
};

/**
 * @route   DELETE /api/teacher/classrooms/:classroomId
 * @desc    Deletes a classroom owned by the teacher.
 * @access  Private (Teacher only) - DANGEROUS ACTION
 */
export const deleteClassroom = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error deleting classroom.", error });
  }
};
/**
 * @route   POST /api/teacher/classrooms/:classroomId/add-student
 * @desc    Manually adds a student to a classroom owned by the teacher.
 * @access  Private (Teacher only)
 */
export const addStudentClassroom = async (req: Request, res: Response) => {
  const teacherId = req.user?.profileId;
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!classroom) {
        throw new Error(
          "Classroom not found or you do not have permission to manage it."
        );
      }
      const studentAccount = await tx.account.findUnique({
        where: { email: studentEmail },
        include: { studentProfile: true },
      });

      if (!studentAccount || !studentAccount.studentProfile) {
        throw new Error("You can only add students from your own institution.");
      }

      const studentId = studentAccount.studentProfile.id;
      const enrollment = await tx.classroomUser.upsert({
        where: {
      ...
      return {
        message: `Successfully added ${studentAccount.studentProfile.name} to the classroom.`,
        enrollment,
      };
    });
...
export const removeStudentFromClassroom = async (
  ...
  if (!classroomId || !studentId) {
    return res
      .status(400)
      .json({ message: "classroomId and studentId are required." });
  }
  ...
  try {
    ...
    return res
      .status(404)
      .json({ message: "Student not found in this classroom.", error });
  }
};
...
export const createChallengeWithAI = async (req: Request, res: Response) => {
  ...
  if (!topic || !challenge_type) {
    return res
      .status(400)
      .json({ message: "Topic and challenge_type are required." });
  }

  try {
    const generatedChallenge: any = await generateChallengeFromTopic({
      topic,
      challenge_type,
    });
...
export const assignChallengeToClassroom = async (
  req: Request,
  res: Response
) => {
  const teacherId = req.user?.profileId;
  ...
  if (!classroomId || !challengeId) {
    return res
      .status(400)
      .json({ message: "classroomId and challengeId are required." });
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Failed to assign challenge.", error });
  }
};
    ...
    const updatedSubmission = await prisma.$transaction(async (tx) => {
      const submission = await tx.submission.findUnique({
        ...
        select: {
          ...
          project: {
            // Also include the project to get its point value if needed
            select: {
      ...
      if (submission.student.institutionId !== teacherInstitutionId) {
        throw new Error(
          "Forbidden: You can only approve submissions from your own institution."
        );
      }
  ...
  try {
    ...
    if (submission.student.institutionId !== teacherInstitutionId) {
      return res.status(403).json({
        message:
          "Forbidden: You can only manage submissions from your own institution.",
      });
    }
